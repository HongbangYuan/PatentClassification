...
import os
from cogtemplate.utils.io_utils import load_model
from cogtemplate.utils.log_utils import logger
from cogtemplate.utils.train_utils import move_dict_value_to_device
...
class Predictor:
    ...
    def predict(self):
        ...
        self.model.eval()
        if self.use_tqdm:
            progress = enumerate(tqdm(self.dev_dataloader, desc="Predicting", leave=False), 1)
        else:
        ...
        with torch.no_grad():
            for step, batch in progress:
                ...
                additional_info.append(list(batch["id"]))
        return [item for sublist in predict_results for item in sublist],[item for sublist in additional_info for item in sublist]